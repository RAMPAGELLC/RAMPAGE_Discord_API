-- Written by tyrannizer.
-- Remix of Badger_Discord_API to be better organized & additional features.
...
function CreateNotification(PlayerId, Style, Event, Message)
    if Config.Notifications.EventsEnabled[Event] == nil or Config.Notifications.EventsEnabled[Event] ~= nil and
        not Config.Notifications.EventsEnabled[Event] then
        return false
    end
    ...
    TriggerClientEvent(Config.Notifications.ResourceName .. ':' .. Config.Notifications.ResourceExport, PlayerId, {
        ...
        [Config.Notifications.Format.style] = Config.Notifications.Format.styles[Style] or
            Config.Notifications.Format.styles["default"],
...
function GetDiscordId(PlayerId)
    for _, id in ipairs(GetPlayerIdentifiers(PlayerId)) do
        if string.match(id, "discord:") then
            return true, string.gsub(id, "discord:", "")
        end
    end
...
function GetDiscordRoles(PlayerId, GuildId)
    ...
    if Config.UseCache and DataCache.UserRoles[discordId] and DataCache.UserRoles[discordId][discordId][guildId] then
        return DataCache.UserRoles[discordId][discordId][guildId]
    end
...
Citizen.CreateThread(function()
    if (GetCurrentResourceName() ~= "RAMPAGE_Discord_API") then
        print("[" .. GetCurrentResourceName() .. "] " ..
                  "IMPORTANT: Resource MUST be named RAMPAGE_Discord_API otherwise some scripts may NOT work properly.");
    end
    ...
    if Guild.code == 200 then
        ...
        print("[RAMPAGE_Discord_API] An error occured while attempting to connect to guild. Error Data: " ..
...
AddEventHandler('RAMPAGE_Discord_API:LoadPlayer', function()
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            local success, discordId = GetDiscordId(source)
            local success2, roles = GetDiscordRoles(source)
            ...
            if success and success2 then
                ...
                if added and added ~= {} then
                    CreateNotification(source, "green", "RoleAdded", added .. " roles has been added to your discord account.")
                end

                if removed and removed ~= {} then
                    CreateNotification(source, "red", "RoelRemoved", added .. " roles has been removed to your discord account.")
                end
            end
        end
    end)
